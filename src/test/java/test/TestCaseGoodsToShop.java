...
public class TestCaseGoodsToShop {
	...
	@Test
	public void goodsToShop() throws InterruptedException {
		/** 商品资料生成商城数据 **/

		dr.switchTo().defaultContent();
		Actions builder = new Actions(dr);
		System.out.println("开始商品资料维护");
		...
		wait.until(
				ExpectedConditions.elementToBeClickable(By
						.xpath("//div[contains(@class,'mini-layout-region-north')]/div[2]/table/tbody/tr[1]/td[2]/span/span/span")))
				.click();
		wait = new WebDriverWait(dr, 30);
		wait.until(
				ExpectedConditions.elementToBeClickable(By
						.xpath("//td[text()[contains(.,'未生成商城数据')]]")))
				.click();
		wait = new WebDriverWait(dr, 30);
		wait.until(
				ExpectedConditions.elementToBeClickable(By
		...
		while (pageNow <= pageNum) {
			...
			System.out.println("判断是否最后一页");
			if(pageNow == pageNum){
				Thread.sleep(3000);
				wait = new WebDriverWait(dr, 60);
				wait.until(
						ExpectedConditions.elementToBeClickable(By
								.xpath("/html/body/div[6]/div/div[2]/div[2]/div[2]/a"))).click();
			}
			wait = new WebDriverWait(dr, 60);
			wait.until(
					ExpectedConditions.elementToBeClickable(By
							.xpath("/html/body/div[4]/div/div[2]/div[2]/div[2]/a")))
					.click();
			wait = new WebDriverWait(dr, 60);
			wait.until(
					ExpectedConditions.elementToBeClickable(By
							.xpath("//span[contains(@class,'mini-pager-next')]")))
					.click();
			Thread.sleep(3000);
			pageNow++;
	...
	public void checkall(WebDriver dr, int pageIndex)
		...
		wait.until(
				ExpectedConditions.elementToBeClickable(By
						.xpath("//span[text()[contains(.,'生成商城数据')]]")))
				.click();
		// 选择商城
		Thread.sleep(3000);
